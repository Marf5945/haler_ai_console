...
//
// 需要真實模型檔案：assets/models/yolox_button_s.mlmodelc
...
func modelBasePath(t *testing.T) string {
	...
	return filepath.Join(projectRoot, "assets", "models", "yolox_button_s")
}
...
func TestCoreMLInferenceEndToEnd(t *testing.T) {
	...
	// ── Step 4: YOLO 後處理 ──
	proposals, decodeErr := DecodeYOLOOutput(raw, width, height, DefaultYOLOXButtonSConfig)
	if decodeErr != nil {
