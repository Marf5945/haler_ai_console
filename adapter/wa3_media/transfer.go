// wa3_media/transfer.go — §9A.12 原檔傳輸引導。
//
// ┌─────────────────────────────────────────────────────────────┐
// │ 匯出媒體時提供軟性 toast 提示，引導使用者以原檔方式傳輸。  │
// │                                                             │
// │ 推薦方式：原檔傳輸 / 附件模式 / 打包保留 sidecar            │
// │ 不推薦：社群平台直傳 / 壓縮模式 / 截圖轉發 / 非 WA3 工具   │
// │                                                             │
...
func GetTransferGuidance() TransferGuidance {
	return TransferGuidance{
		Recommended: []string{
			...
			"使用 WA3-aware app 進行編輯/匯出",
		},
		NotRecommended: []string{
			"社群平台直接上傳圖片作為保存副本",
			"通訊軟體壓縮圖片模式",
			"截圖轉發",
			"非 WA3-aware 濾鏡/美化/優化/轉換工具",
			"未知工具的音訊正規化或轉碼",
		},
		UIMessage: "為保留 WA3 原始簽章與訓練安全狀態，請使用「原檔 / 文件 / 附件」方式傳送。\n不要使用會壓縮圖片或音訊的平台傳送方式。",
	}
}

// ShouldShowTransferToast 判斷匯出時是否應顯示傳輸引導 toast。
// 只要媒體有 WA3 驗證資訊，就建議提示。
