...
func mergePromptActionTags(tags []string) []string {
	...
	var all []string
	for _, tag := range []string{"讀取", "列出", "開啟", "寫入", "儲存", "網路", "搜尋", "展開", "匯入", "匯出", "git", "排程", "提問", "選項", "重試", "輸出", "紀念照"} {
		if seen[tag] {
			continue
		}
		seen[tag] = true
...
func canonicalPromptActionTag(tag string) string {
	...
	case "本機搜尋", "搜尋", "查找", "查詢", "search", "find", "query":
		return "搜尋"
	case "紀念照", "合照", "拍照", "紀念", "keepsake", "photo":
		return "紀念照"
	case "讀取", "read", "查看":
		return "讀取"
	case "列出":
