...
// UpdateManifest 改寫既有 skill 的 skill_manifest.json（TASK 31 / Phase 2「總是允許」持久化）。
// 只更新 manifest 本身（含 lifecycle 與 canonical hash），不動資源結構。
func (svc *ArchiveService) UpdateManifest(m *SkillManifest) error {
	if m == nil || m.SkillID == "" {
		return fmt.Errorf("skill_step: UpdateManifest: nil manifest 或空 skill_id")
	}
	svc.mu.Lock()
	defer svc.mu.Unlock()
	return SaveManifest(svc.skillsDir(m.SkillID), m) // SaveManifest 會重算 canonicalHash
}

// SavePendingDraft 將 BuildSkillDraft 產生的草稿寫入 data/skills/<skill_id>/。
// 這條路徑只保存 manifest + 最小 README，不匯入外部資源；pending_skill 可被候選，
// draft_candidate 保留在磁碟上供後續修正，但不應顯示為工具。
func (svc *ArchiveService) SavePendingDraft(m *SkillManifest) (*SkillManifest, error) {
	if m == nil || m.SkillID == "" {
		return nil, fmt.Errorf("skill_step: SavePendingDraft: nil manifest 或空 skill_id")
	}
	svc.mu.Lock()
	defer svc.mu.Unlock()

	skillDir := svc.skillsDir(m.SkillID)
	if err := os.MkdirAll(skillDir, 0o700); err != nil {
		return nil, fmt.Errorf("skill_step: create pending skill dir: %w", err)
	}
	if m.DescriptionDoc == "" {
		m.DescriptionDoc = "README.md"
	}
	readmePath := filepath.Join(skillDir, m.DescriptionDoc)
	if _, err := os.Stat(readmePath); os.IsNotExist(err) {
		title := m.DisplayName
		if title == "" {
			title = m.SkillID
		}
		body := fmt.Sprintf("# %s\n\nPending skill draft generated by AI Console.\n", title)
		if err := os.WriteFile(readmePath, []byte(body), 0o600); err != nil {
			return nil, fmt.Errorf("skill_step: write pending README: %w", err)
		}
	}
	if err := SaveManifest(skillDir, m); err != nil {
		return nil, err
	}
	saved := *m
	return &saved, nil
